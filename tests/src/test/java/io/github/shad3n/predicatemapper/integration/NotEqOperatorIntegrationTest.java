...
public class NotEqOperatorIntegrationTest {
    ...
    @Test
    void testNotEqOperator_matchMultiple() {
    ...
    @Test
    void testNotEqOperator_noMatch() {
        UserFilter filter = new UserFilter();
        filter.setNotUsername("nonexistent");
    ...
    @Test
    void testNotEqOperator_nullValue() {
        UserFilter filter = new UserFilter();
        filter.setNotUsername(null);

        List<User> users = (List<User>) userRepository.findAll(userPredicateMapper.filter(filter));

        assertThat(users).hasSize(4); // Null filter field means no filtering applied
    }
}
